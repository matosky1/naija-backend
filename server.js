...
app.post("/api/square/charge", async (req, res) => {
  try {
    const { sourceId, amount, postalCode, discountCode, email, shippingAddress } = req.body;

    console.log("🧾 Incoming payment:", { amount, discountCode, email, shippingAddress });
...
Amount: $${finalAmount.toFixed(2)} CAD
...
` : 'No shipping address provided'}
    ...
    // ✅ FIX: Return proper error status (400) instead of 200
    res.status(400).json({
      payment: null,
      errors
    });
